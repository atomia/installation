...
# 2. Generate CA
# 3. Generate keys
# 4. Generate all CSR:s
# 5. Sign certificates

variables = ""
file = File.new("/etc/puppet/manifests/variables.pp", "r")
...
# Certificate info
appdomain = /\$appdomain.*=(.*)/.match(variables)[1].gsub('"','').gsub(' ', '')
ca_name="Atomia #{appdomain} Root Cert"

actiontrail = /\$actiontrail.*=(.*)/.match(variables)[1].gsub('"','').gsub(' ', '')
login = /\$login.*=(.*)/.match(variables)[1].gsub('"','').gsub(' ', '')
order = /\$order.*=(.*)/.match(variables)[1].gsub('"','').gsub(' ', '')
billing  = /\$billing .*=(.*)/.match(variables)[1].gsub('"','').gsub(' ', '')
admin = /\$admin =(.*)/.match(variables)[1].gsub('"','').gsub(' ', '')
hcp = /\$hcp.*=(.*)/.match(variables)[1].gsub('"','').gsub(' ', '')
automationserver = /\$automationserver.*=(.*)/.match(variables)[1].gsub('"','').gsub(' ', '')
sts = "sts"
userapi = "userapi"
...
system("openssl req -new -key \"private/login.key\" -subj \"/C=/ST=/L=/O=/CN=#{actiontrail}.#{login}\" -out \"csr/login.csr\"")
